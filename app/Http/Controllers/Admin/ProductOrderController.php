...
class ProductOrderController extends Controller
{
    ...
    public function completed(Request $request)
    {
        ...
        $po->save();
        $table = Table::where('table_no', $po->table_number)->first();
...
if ($table) {
    ...
    public function fetchOrders(Request $request)
    {
        ...
        $orders = ProductOrder::when($search, function ($query, $search) {
                return $query->where('order_number', 'LIKE', '%' . $search . '%');
            })
            ->when($type, function ($query, $type) {
                return $query->where('type', $type);
    ...
    public function qrPrint(Request $request)
    {
        ...
        \QrCode::size(150)
            ->color(0, 0, 0)
            ->format('svg')
            ->generate($url, public_path('assets/front/img/' . $fileName));

        return url(public_path('assets/front/img/' . $fileName));
    }
