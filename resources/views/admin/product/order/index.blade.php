      ...
      <div class="card">
                ...
                <div class="row no-gutters">
                    <div class="col-lg-1">
                        <div class="form-group px-0">
                            <label for="">طريقة الطلب</label>
                            <select name="orders_from" class="form-control" onchange="document.getElementById('searchForm').submit()">
                                <option value="" {{empty(request()->input('orders_from')) ? 'selected' : ''}}>الكل</option>
                                <option value="website" {{request()->input('orders_from') == 'website' ? 'selected' : ''}}>الموقع Menu</option>
                                <option value="qr" {{request()->input('orders_from') == 'qr' ? 'selected' : ''}}>Qr Menu</option>
                                <option value="pos" {{request()->input('orders_from') == 'pos' ? 'selected' : ''}}>الكاشير</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-lg-2">
                        <div class="form-group px-0">
                            ...
                            <select name="serving_method" class="form-control" onchange="document.getElementById('searchForm').submit()">
                                <option value="" {{empty(request()->input('orders_from')) ? 'selected' : ''}}>الكل</option>
                                <option value="on_table" {{request()->input('serving_method') == 'on_table' ? 'selected' : ''}}>محلي</option>
                                <option value="pick_up" {{request()->input('serving_method') == 'pick_up' ? 'selected' : ''}}>سفري</option>
                    ...
                    <div class="col-lg-1">
                        <div class="form-group px-0">
                            ...
                            <select name="order_status" id="" class="form-control" onchange="document.getElementById('searchForm').submit()">
                                <option value="" {{empty(request()->input('order_status')) ? 'selected' : ''}}>الكل</option>
                                <option value="pending" {{request()->input('order_status') == 'pending' ? 'selected' : ''}}>معلقة</option>
                                <option value="received" {{request()->input('order_status') == 'received' ? 'selected' : ''}}>تم إستلامها</option>
                                <option value="preparing" {{request()->input('order_status') == 'preparing' ? 'selected' : ''}}>قيد التحضير</option>
                                ...
                                <option value="ready_to_serve" {{request()->input('order_status') == 'ready_to_serve' ? 'selected' : ''}}>جاهز للتقديم</option>
                                <option value="served" {{request()->input('order_status') == 'served' ? 'selected' : ''}}>تم تقديمه</option>
                                @endif
                            </select>
                        </div>
                    </div>
                    <div class="col-lg-1">
                        <div class="form-group px-0">
                            ...
                            <select name="payment_status" class="form-control" onchange="document.getElementById('searchForm').submit()">
                                ...
                                <option value="Pending" {{request()->input('payment_status') == 'Pending' ? 'selected' : ''}}>معلقة</option>
                                <option value="Completed" {{request()->input('payment_status') == 'Completed' ? 'selected' : ''}}>مكتملة</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-lg-1">
                        <div class="form-group px-0">
                            <label for="">طلب مكتمل</label>
                            <select name="completed" class="form-control" onchange="document.getElementById('searchForm').submit()">
                                <option value="">الكل</option>
                                <option value="yes" {{request()->input('completed') == 'yes' ? 'selected' : ''}}>نعم</option>
                                <option value="no" {{request()->input('completed') == 'no' ? 'selected' : ''}}>لا</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-lg-2">
                    ...
                    <div class="col-lg-2">
                        <div class="form-group px-0">
                            ...
                            <input class="form-control" type="text" name="search" placeholder="البحث عن طريق رقم الطلب" value="{{request()->input('search') ? request()->input('search') : '' }}">
                        </div>
                    </div>
                    <div class="col-lg-2">
                        <div class="form-group px-0">
        ...
        <div class="card-body">
          <div class="row">
            <div class="col-lg-12">
              @if (count($orders) == 0)
                <h3 class="text-center">ليس هناك أي طلبات</h3>
              @else
                <div id="refreshOrder">
                      ...
                      <table class="table table-striped mt-3">
                        <thead>
                          <tr>
                            ...
                            <th scope="col">طريقة التقديم</th>
                            <th scope="col">حالة الدفع</th>
                            <th scope="col">حالة الطلب</th>
                            <th scope="col">هل الطلب مكتمل</th>
                            {{-- <th scope="col">Gateway</th> --}}
                            <th scope="col">تاريخ ووقت الطلب</th>
                                ...
                                @if ($order->type == 'pos' || $order->gateway_type == 'offline')
                                    <form id="paymentForm{{$order->id}}" class="d-inline-block" action="{{route('admin.product.order.payment')}}" method="post">
                                        ...
                                        " name="payment_status" onchange="document.getElementById('paymentForm{{$order->id}}').submit();">
                                                <option value="Pending" {{$order->payment_status == 'Pending' ? 'selected' : ''}}>معلقة</option>
                                                <option value="Completed" {{$order->payment_status == 'Completed' ? 'selected' : ''}}>مكتملة</option>
                                        </select>
                                    </form>
                                @else
                              ...
                              <td>
                                <form id="statusForm{{$order->id}}" class="d-inline-block" action="{{route('admin.product.orders.status')}}" method="post">
                                  ...
                                  " name="order_status" onchange="document.getElementById('statusForm{{$order->id}}').submit();">
                                    <option value="pending" {{$order->order_status == 'pending' ? 'selected' : ''}}>معلقة</option>
                                    <option value="received" {{$order->order_status == 'received' ? 'selected' : ''}}>تم استلامها</option>
                                    ...
                                    @if ($order->serving_method == 'on_table')
                                    <option value="ready_to_serve" {{$order->order_status == 'ready_to_serve' ? 'selected' : ''}}>جاهز للتقديم</option>
                                    <option value="served" {{$order->order_status == 'served' ? 'selected' : ''}}>تم تقديمه</option>
                                    @endif

                                    <option value="cancelled" {{$order->order_status == 'cancelled' ? 'selected' : ''}}>تم إلغائه</option>
                                  </select>
                                </form>
                              </td>
                              <td>
                                <form id="completeForm{{$order->id}}" class="d-inline-block" action="{{route('admin.product.order.completed')}}" method="post">
                                    ...
                                    " name="completed" onchange="document.getElementById('completeForm{{$order->id}}').submit();">
                                            <option value="no" {{empty($order->completed) || $order->completed == 'no' ? 'selected' : ''}}>لا</option>
                                            <option value="yes" {{$order->completed == 'yes' ? 'selected' : ''}}>نعم</option>
                                    </select>
                                  </form>
                              </td>
                              {{-- <td class="text-capitalize">
